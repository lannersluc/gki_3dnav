...
#ifndef TERRAINMAPPLANNINGENVIRONMENT_MPH_H_
#define TERRAINMAPPLANNINGENVIRONMENT_MPH_H_
...
class PlanningEnvironmentMPH : public PlanningEnvironment
{
public:
    PlanningEnvironmentMPH(const CollisionChecker* checker);
    virtual ~PlanningEnvironmentMPH();
    ...
    void precomputeHeuristicActions(bool fullExpandHeuristic = false);
    void testStateDiscretization(const tf::Pose& pose, std::vector<tf::Pose>& rotated);
    void setHeuristicTable(const StateIntMap& heuristicTable){stateHeuristicTable = heuristicTable;}
    const StateIntMap& getHeuristicTable() const {return stateHeuristicTable;}
...
private:
    ...
    void reset();
    void updateHeuristic(State* state, int cost);

    State* goalState;
    State* startState;
    IdStateMap idToState;
    StateIntMap stateToId;
    StateIntMap stateHeuristicTable;
    bool computingHeuristic;
    bool fullExpandHeuristic;
    ...
    std::vector<Action> heuristicActions;
    std::vector<sbpl_2Dpt_t>heuristicFootprintPolygon;
...
#endif /* TERRAINMAPPLANNINGENVIRONMENT_MPH_H_ */
