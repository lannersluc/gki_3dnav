...
#include "gki_3dnav_planner/PlanningEnvironmentMPH.h"
...
PlanningEnvironmentMPH::PlanningEnvironmentMPH(const CollisionChecker* checker)
{
    ...
    bestHeuristicValue = INT_MAX;
    forEach (const tf::Point& point, checker->getBaseFootprintPolygon())
    {
        footprintPolygon.push_back(sbpl_2Dpt_t(point.x(), point.y()));
    }
    forEach (const tf::Point& point, checker->getHeuristicBaseFootprintPolygon())
    {
        heuristicFootprintPolygon.push_back(sbpl_2Dpt_t(point.x(), point.y()));
    }
    actionwidth = 0;
    computingHeuristic = false;
    fullExpandHeuristic = false;

    ROS_INFO("%s ends", __PRETTY_FUNCTION__);
}

PlanningEnvironmentMPH::~PlanningEnvironmentMPH()
{
    startState = NULL;
    goalState = NULL;
...
bool PlanningEnvironmentMPH::readinCell(sbpl_xy_theta_cell_t* cell, FILE* fIn)
{
    char sTemp[60];
...
bool PlanningEnvironmentMPH::readinPose(sbpl_xy_theta_pt_t* pose, FILE* fIn, double resolutionCorrectionFactor)
{
    char sTemp[60];
...
void PlanningEnvironmentMPH::precomputeHeuristicActions(bool fullExpandHeuristic)
{
    computingHeuristic = true;
    this->fullExpandHeuristic = fullExpandHeuristic;
    ...
    {
        heuristicActions.push_back(Action());
        Action& action = heuristicActions.back();
        ...
        get_2d_motion_cells(heuristicFootprintPolygon, intermediatePoses, &discreteAction.intersectingcellsV, params::motionPrimitiveResolution);
        std::set<sbpl_2Dcell_t> initialCells;
        get_2d_footprint_cells(heuristicFootprintPolygon, &initialCells, sourcepose, params::motionPrimitiveResolution);
        std::set<sbpl_2Dcell_t> finalCells;
        get_2d_footprint_cells(heuristicFootprintPolygon, &finalCells, destinationPose, params::motionPrimitiveResolution);
        action.updateFromDiscretePoses();
    }
}

//here motionprimitivevector contains actions for all angles
void PlanningEnvironmentMPH::precomputeActionswithCompleteMotionPrimitive(std::vector<SBPL_xytheta_mprimitive>* motionprimitiveV)
{
...
bool PlanningEnvironmentMPH::initializeSearch(tf::Pose start, tf::Pose goal)
{
//    computingHeuristic = computeHeuristic;
    ...
    expandedIds.clear();
    if (computingHeuristic)
    {
        startState = findOrCreateState(goal);
        startState->footprint = goalEdgeCells;
        if (fullExpandHeuristic)
        {
            tf::Pose extremeFarAway(tf::createQuaternionFromYaw(0), tf::Point(1000, 1000, 0));
            goalState = findOrCreateState(extremeFarAway);
        }
        else
        {
            goalState = findOrCreateState(start);
        }
        goalState->footprint = startEdgeCells;
        updateHeuristic(startState, 0);
    }
    else
    {
...
bool PlanningEnvironmentMPH::convertPath(const std::vector<int>& pathIds, std::vector<tf::Pose>& pathPoses, std::vector<Cost>* pathCosts) const
{
...
bool PlanningEnvironmentMPH::convertState(int stateId, tf::Pose* statePose) const
{
...
bool PlanningEnvironmentMPH::InitializeEnv(const char* sEnvFile)
{
 ...
 */
bool PlanningEnvironmentMPH::InitializeMDPCfg(MDPConfig *MDPCfg)
{
 ...
 */
int PlanningEnvironmentMPH::GetFromToHeuristic(int FromStateID, int ToStateID)
{
 ...
 */
int PlanningEnvironmentMPH::GetGoalHeuristic(int stateID)
{
 ...
 */
int PlanningEnvironmentMPH::GetStartHeuristic(int stateID)
{
    return getFromToHeuristic(startState, idToState.find(stateID)->second);
}

int PlanningEnvironmentMPH::getFromToHeuristic(const State* fromState, const State* toState)
{
    if (! computingHeuristic)
    {
        // actual search: lookup heuristic value in table
        State heuristicKey = *fromState;
        heuristicKey.yaw = 0;
        StateIntMap::const_iterator it = stateHeuristicTable.find(&heuristicKey);
        if (it != stateHeuristicTable.end())
        {
            return it->second;
        }
//        ROS_WARN("heuristic table lookup failed");
//        static int fail_id = 0;
//        checker->visualizePoint(fromState->pose.getOrigin(), checker->grayColor, "lookup_failed", fail_id++);
...
State* PlanningEnvironmentMPH::findOrCreateState(const tf::Pose& pose)
{
...
void PlanningEnvironmentMPH::updateHeuristic(State* state, int cost)
{
    StateIntMap::const_iterator it = stateHeuristicTable.find(state);
    if (it != stateHeuristicTable.end())
    {
    ...
    stateHeuristicTable.insert(std::make_pair(state, cost));
}
 ...
 */
void PlanningEnvironmentMPH::GetSuccs(int SourceStateID, std::vector<int>* successorIds, std::vector<int>* costs)
{
    successorIds->clear();
    costs->clear();
...
//    checker->visualizePose(current->pose);
    ...
    if (computingHeuristic)
    {
        // get cost for current state as heuristic
        StateIntMap::const_iterator it = stateHeuristicTable.find(current);
        ROS_ASSERT_MSG(it != stateHeuristicTable.end(), "heuristic lookup failed.");
        int currentCost = it->second;

        forEach(const Action& action, heuristicActions)
        {
            ...
            updateHeuristic(newState, currentCost + motionCost.total / sqrt(2));
        }
    }
    else
    {
 ...
 */
void PlanningEnvironmentMPH::GetPreds(int TargetStateID, std::vector<int>* PredIDV, std::vector<int>* CostV)
{
    ROS_FATAL("%s not supported.", __PRETTY_FUNCTION__);
}
 ...
 * \brief see comments for GetSuccs functon
 */
void PlanningEnvironmentMPH::SetAllActionsandAllOutcomes(CMDPSTATE* state)
{
    ROS_FATAL("%s not supported.", __PRETTY_FUNCTION__);
}
 ...
 * \brief see comments for GetSuccs functon
 */
void PlanningEnvironmentMPH::SetAllPreds(CMDPSTATE* state)
{
    ROS_FATAL("%s not supported.", __PRETTY_FUNCTION__);
}
 ...
 */
int PlanningEnvironmentMPH::SizeofCreatedEnv()
{
 ...
 */
void PlanningEnvironmentMPH::PrintState(int stateID, bool bVerbose, FILE* fOut)
{
    IdStateMap::const_iterator stateIt = idToState.find(stateID);
    if (stateIt != idToState.end())
...
void PlanningEnvironmentMPH::PrintState(const State* state, bool bVerbose, FILE* fOut)
{
    ROS_INFO("%d [%d %d %d %d]", state->id, state->x, state->y, state->z, state->yaw);
//    PrintPose(state->pose);
}
void PlanningEnvironmentMPH::PrintPose(const tf::Pose& pose)
{
 ...
 */
void PlanningEnvironmentMPH::PrintEnv_Config(FILE* fOut)
{
    ROS_INFO("%s begins", __PRETTY_FUNCTION__);
...
void PlanningEnvironmentMPH::testStateDiscretization(const tf::Pose& pose, std::vector<tf::Pose>& expanded)
{
