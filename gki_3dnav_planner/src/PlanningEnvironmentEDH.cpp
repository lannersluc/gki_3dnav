...
#include "gki_3dnav_planner/PlanningEnvironmentEDH.h"
...
PlanningEnvironmentEDH::PlanningEnvironmentEDH(const CollisionChecker* checker)
{
...
PlanningEnvironmentEDH::~PlanningEnvironmentEDH()
{
    ROS_INFO("%s begins", __PRETTY_FUNCTION__);
    startState = NULL;
    goalState = NULL;
...
void PlanningEnvironmentEDH::readMotionPrimitives(const std::string& file)
{
...
bool PlanningEnvironmentEDH::readinMotionPrimitive(SBPL_xytheta_mprimitive* pMotPrim, FILE* fIn, double resolutionCorrectionFactor)
{
    char sTemp[1024];
    int dTemp;
    char sExpected[1024];
...
bool PlanningEnvironmentEDH::readinCell(sbpl_xy_theta_cell_t* cell, FILE* fIn)
{
    char sTemp[60];
...
bool PlanningEnvironmentEDH::readinPose(sbpl_xy_theta_pt_t* pose, FILE* fIn, double resolutionCorrectionFactor)
{
    char sTemp[60];
...
void PlanningEnvironmentEDH::precomputeActionswithCompleteMotionPrimitive(std::vector<SBPL_xytheta_mprimitive>* motionprimitiveV)
{
...
bool PlanningEnvironmentEDH::initializeSearch(tf::Pose start, tf::Pose goal)
{
    ROS_INFO("%s begins", __PRETTY_FUNCTION__);
...
bool PlanningEnvironmentEDH::convertPath(const std::vector<int>& pathIds, std::vector<tf::Pose>& pathPoses, std::vector<Cost>* pathCosts) const
{
...
bool PlanningEnvironmentEDH::convertState(int stateId, tf::Pose* statePose) const
{
...
bool PlanningEnvironmentEDH::InitializeEnv(const char* sEnvFile)
{
 ...
 */
bool PlanningEnvironmentEDH::InitializeMDPCfg(MDPConfig *MDPCfg)
{
 ...
 */
int PlanningEnvironmentEDH::GetFromToHeuristic(int FromStateID, int ToStateID)
{
 ...
 */
int PlanningEnvironmentEDH::GetGoalHeuristic(int stateID)
{
 ...
 */
int PlanningEnvironmentEDH::GetStartHeuristic(int stateID)
{
    return getFromToHeuristic(startState, idToState.find(stateID)->second);
}

int PlanningEnvironmentEDH::getFromToHeuristic(const State* fromState, const State* toState)
{
    ...
    return 1000 * distance / params::linearVelocity;
}

State* PlanningEnvironmentEDH::findOrCreateState(const tf::Pose& pose)
{
 ...
 */
void PlanningEnvironmentEDH::GetSuccs(int SourceStateID, std::vector<int>* successorIds, std::vector<int>* costs)
{
    successorIds->clear();
    costs->clear();
 ...
 */
void PlanningEnvironmentEDH::GetPreds(int TargetStateID, std::vector<int>* PredIDV, std::vector<int>* CostV)
{
    ROS_FATAL("%s not supported.", __PRETTY_FUNCTION__);
}
 ...
 * \brief see comments for GetSuccs functon
 */
void PlanningEnvironmentEDH::SetAllActionsandAllOutcomes(CMDPSTATE* state)
{
    ROS_FATAL("%s not supported.", __PRETTY_FUNCTION__);
}
 ...
 * \brief see comments for GetSuccs functon
 */
void PlanningEnvironmentEDH::SetAllPreds(CMDPSTATE* state)
{
    ROS_FATAL("%s not supported.", __PRETTY_FUNCTION__);
}
 ...
 */
int PlanningEnvironmentEDH::SizeofCreatedEnv()
{
 ...
 */
void PlanningEnvironmentEDH::PrintState(int stateID, bool bVerbose, FILE* fOut)
{
    IdStateMap::const_iterator stateIt = idToState.find(stateID);
    if (stateIt != idToState.end())
...
void PlanningEnvironmentEDH::PrintState(const State* state, bool bVerbose, FILE* fOut)
{
    ROS_INFO("%d [%d %d %d %d]", state->id, state->x, state->y, state->z, state->yaw);
//    PrintPose(state->pose);
}
void PlanningEnvironmentEDH::PrintPose(const tf::Pose& pose)
{
 ...
 */
void PlanningEnvironmentEDH::PrintEnv_Config(FILE* fOut)
{
    ROS_INFO("%s begins", __PRETTY_FUNCTION__);
...
void PlanningEnvironmentEDH::testStateDiscretization(const tf::Pose& pose, std::vector<tf::Pose>& expanded)
{
